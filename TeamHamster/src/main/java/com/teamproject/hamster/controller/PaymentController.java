...
public class PaymentController {
    ...
    private ArrayList<ProductDTO> payments = new ArrayList<>();

    Scanner sc=new Scanner(System.in);

    public void checkCoupon() {
        System.out.print("회원가입 쿠폰이 있습니다. 사용하시겠습니까? (사용 : Y) ");
    ...
    public void print(ArrayList<ProductDTO> product) {
        ...
        payments.addAll(product);
        for(int i=0;i<product.size();i++) {
            total_money+=payments.get(i).getPrice();
        }
        ...
        if(num==1) {
            ...
            for (int i =0; i < payments.size(); i++){
                System.out.println(payments.get(i).getName() + payments.get(i).getSize() + "입니다.");
